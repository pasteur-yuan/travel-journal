...
現況：29 個地區頁、514 列地點資料、1204 項測試通過（另有 2 項略過，見 1.1）

---

## 1. 內容資料

### 1.1 旅行筆記 — ✅ 獨立成 `region-notes.js`，2026/03 九州行程（福岡／大分／佐賀）已加入
這個項目經過兩次大改：先是把全站舊有的假筆記資料（含 22 個地區 HTML 裡寫死的「待補充」佔位筆記）全部清空，接著你指出旅行筆記未來會被其他頁面參考，資料不該放在 `region-detail.js` 裡，要求獨立成一支 JS——於是新增 `assets/js/region-notes.js`，宣告全域 `const regionNotes`（跟 `themes.js` 宣告 `const themes` 同一種模式，純 classic script、不用 `type="module"`，符合全站 `file://` 直接開啟的需求），29 個地區頁的 `<script>` 都補上這一行，位置在 `theme-switcher.js` 之後、`region-detail.js` 之前。

**資料結構**：`regionNotes[地區]` 是陣列，依顯示順序排列，元素是 `{ label, description, itinerary? }`——不再區分「第一則」跟「其餘」，也不再用兩個獨立 map（`regionContent[地區].note` 跟 `regionAdditionalContent[地區].notes`）分開放，全部統一在同一個地方。`itinerary` 選填，沒有這趟旅程的實際停留點資料時就不寫這個欄位，modal 就不顯示停留點清單，也不會顯示地名／資訊／交通方式的四欄表格——不編造內容。

**改的過程中發現一個先於這整個工作階段就存在的真實 bug**：舊架構裡，「第二則以後的筆記」跟 spots/food/stays 共用同一個 `appendItems()`／`createRegionContentItem('list')`，這個共用函式是三欄位（標籤、標題、敘述），但筆記資料只有兩欄位（標籤、敘述）。結果第二則以後的筆記，敘述文字被塞進了粗體的 `<h3>`，原本該顯示敘述的 `<p>` 卻永遠是空的——用還原到清空前的舊資料實測、截圖證實了這個現象。這極可能就是你在這個工作階段一開始回報「旅行筆記的資料呈現起來怪怪的」的真正原因（當時選擇直接清空重來，沒有繼續追根因）。新架構已經修正：`region-notes.js` 的每一筆筆記都用專屬的 `createNoteItem()`（`<span>`／`<p>` 兩欄，不經過三欄位的共用函式），不分第一則或其後，全部走同一條渲染路徑，這個 bug 不會再發生。

**這次加入的真實資料**：2026/03/21–03/31 的九州行程（86 列原始資料），橫跨福岡、大分、佐賀三個地區，依實際地理位置拆開（跟上一輪處理「去趣行程總覽.md」時的方法完全相同，逐列比對過確認一致）：
- **福岡**：11 天、79 站，標籤 `2026 / 03`。
- **大分**：1 天（3/29 湯布院／金鱗湖）、4 站，標籤 `2026 / 03`。
- **佐賀**：1 天（3/25 新鳥栖車站旁購物中心）、1 站，標籤 `2026 / 03`——這則筆記原本沿用一句舊的通用敘述（「沿著唐津的松原與有明海的潮汐移動...」），但跟真實行程（只有新鳥栖一站，跟唐津、有明海完全無關）對不上，這次改寫成符合實際行程的新句子。
...
**測試**：`notesOrder`（原本檢查「插入順序沒被打亂」，現在改成用 `region-notes.js` 的原始碼當比對基準，驗證畫面顯示順序跟 `regionNotes` 陣列順序一致）、`itineraryContent`、`itineraryAccessibility` 三個群組，全部不寫死是哪個地區哪個標籤，逐一動態搜尋有資料的筆記，沒有資料的斷言才顯示「略過」。這次福岡的真實資料讓大部分先前「略過」的斷言變成真正執行並通過。`tests/suites/region-html-skeleton.mjs` 的 script 清單也同步加了 `region-notes.js`。全站測試 1204 通過、2 略過（都是「沒有行程資料的筆記」這類、跟這次加的福岡/大分/佐賀資料無關的斷言，非 bug）。
...
每趟行程要同時給兩件事，缺一不可：一句話的筆記文字（年月標籤＋敘述）跟上面這份行程明細，兩者一起寫進 `assets/js/region-notes.js` 的 `regionNotes[地區]` 陣列，同一個物件的 `label`／`description`／`itinerary` 三個欄位。如果同一趟旅程橫跨多個地區，或同一天橫跨兩個地區，會依實際地點正確拆開到各自的地區陣列裡——這個邏輯已經驗證過沒問題（例如 2025 年 4-5 月九州行程同時觸及熊本、宮崎、大分、福岡；2026 年 3 月這趟橫跨福岡、大分、佐賀），之後補資料不需要重新設計，照著同一份表格格式給即可。
...
### 1.4 所有地區的旅行筆記
**狀態**：跟 1.1 是同一件事，做法與資料格式都寫在 1.1 裡，這裡不重複。目前只有福岡／大分／佐賀有真實資料（2026/03 九州行程），其餘 26 個地區的 `regionNotes[地區]` 都還是空陣列 `[]`，等你逐趟提供。
