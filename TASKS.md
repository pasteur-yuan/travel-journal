...
現況：29 個地區頁、514 列地點資料、1211 項測試通過

---

## 1. 內容資料

### 1.1 旅行筆記 — ✅ 從地區頁搬到首頁時間軸，2026/03 九州行程已加入
這個項目經過三次大改。先是把地區頁上舊有的假筆記資料清空；接著把資料獨立成 `region-notes.js`，準備讓地區頁以外的頁面也能參考；最後你決定整個方向要換——**地區頁不再顯示旅行筆記**，改成在**首頁時間軸**點擊資訊卡時，用原本地區頁筆記那個彈出視窗（時間軸式停留點清單）顯示行程細節。`region-notes.js` 已刪除，取代成 `assets/js/trips.js`（全域 `const trips`，跟 `themes.js` 宣告 `const themes` 同一種模式），`index.html` 的 `.timeline-track` 現在是空的，時間軸節點與旅程彈窗完全由這份資料動態產生，不再手寫在 HTML 裡；地區頁回到四個章節（`overview` / `spots` / `food` / `stays`），沒有 `#notes` 了。

**資料結構**：`trips` 是一個陣列（不是照地區分），元素是 `{ label, date, country, regions, teaser, description, itinerary }`。關鍵決定：**一趟旅程即使橫跨多個地區，也不拆成多個時間軸節點**——`itinerary` 是同一份連貫的完整行程，不依地區切開。`regions` 是這趟旅程實際踏過的地區 tag 陣列，只用來讓「已探索」「旅行足跡」的地區數統計反映真實數字（即使畫面上只有一個節點）；時間軸卡片的 `<strong>` 地區名只顯示 `regions[0]`（視為主要地區）——`.timeline-card` 是固定高度、`overflow: hidden` 的小卡片，全部地區名串起來會換行把日期擠出可視範圍，撞到這個問題後才拆出 `teaser`（卡片短句）與 `description`（彈窗完整敘述）兩個欄位。

**過程中發現一個先於這整個工作階段就存在的真實 bug**（雖然現在這塊功能已經搬家，但值得記錄）：舊架構裡「第二則以後的筆記」跟 spots/food/stays 共用同一個三欄位的 `appendItems()`，但筆記資料只有兩欄位，結果敘述文字被塞進粗體 `<h3>`、該顯示的 `<p>` 卻永遠空白——用還原到更早的舊資料實測、截圖證實了這個現象，這極可能就是你在這個工作階段一開始回報「旅行筆記的資料呈現起來怪怪的」的真正原因。新架構下這個問題已經不存在。

**這次加入的真實資料**：2026/03/21–03/31 的九州行程（86 列原始資料，扣掉跳過與省略後 84 站、11 天），一個時間軸節點，標籤 `2026 / 03`，`regions` 標記為 `['福岡', '大分', '佐賀']`（實際依地理位置：福岡 11 天 79 站、大分 1 天 4 站／3/29 湯布院金鱗湖、佐賀 1 天 1 站／3/25 新鳥栖車站旁購物中心，逐列比對過跟上一輪處理「去趣行程總覽.md」時的分法完全一致）。`description` 改寫成涵蓋整趟行程（博多市區＋湯布院一日遠足＋新鳥栖短暫停留），不是只描述福岡的片段。

**兩個已知的省略／低信心點**：
- 3/21 08:00「桃園國際機場第二航廈」（出發地）— 跳過，不屬於任何日本地區。
- 3/27「唐戶市場」（下關市、山口縣）— 站上沒有山口縣的地區頁，維持省略，不臆測併入福岡。
- 3/23「746-1 Kanamaru」— 地名本身查不出明確地點，低信心先歸到福岡。

**測試**：新增 `tests/suites/timeline-modal.mjs`（節點數對應 `trips.length`、資訊卡展開後點擊卡片開彈窗、彈窗內容天數/站數/第一站符合資料、鍵盤與觸控可及性），刪除整份 `region-itinerary.mjs`（地區頁筆記功能已經搬走）。`timeline-desktop.mjs`／`timeline-mobile.mjs`／`timeline-data.mjs`／`home-map.mjs` 這幾份原本大量假設 5 筆示範假資料的地區名／日期／「旅行節點」統計，全部改成不寫死內容——新增 `tests/timeline-fixtures.mjs` 共用工具，用注入 `<script>trips.push(...)</script>`（在 `trips.js` 執行後、`main.js` 執行前）取代原本直接把 `<a class="timeline-entry">` 塞進 HTML 的做法，因為 `main.js` 現在只讀 `trips`、不再讀 HTML 裡的節點。`region-html-skeleton.mjs`／`region-content.mjs`／`region-modal.mjs` 改回四章節、拿掉 notes 相關的斷言分支。全站測試維持全綠。
...
新增一趟旅程要同時給：年月標籤、國家（含國旗）、實際踏過的地區清單（tag，供統計用）、卡片短句（teaser，約 10 字內，太長會把卡片上的日期擠出可視範圍）、彈窗完整敘述（description，涵蓋整趟行程）、以及上面這份完整行程明細（itinerary，不依地區拆分，同一趟旅程橫跨多個地區也是同一份連貫清單）。全部寫進 `assets/js/trips.js` 的 `trips` 陣列，加一筆就好，時間軸節點與彈窗會自動出現。
...
### 1.4 更多旅行筆記
**狀態**：跟 1.1 是同一件事，做法與資料格式都寫在 1.1 裡，這裡不重複。旅行筆記現在是「一趟旅程一筆」寫進 `assets/js/trips.js` 的 `trips` 陣列，不再照地區分——目前只有 2026/03 九州行程這一筆，你之後給的每一趟新行程都會在首頁時間軸多一個節點。
...
### 1.8 首頁時間軸的旅程 — ✅ 機制已經換了，不是補資料就好
**原本的狀態**：時間軸手寫了 5 筆假資料（北海道、東京、福岡、首爾、南島），但站上已有 29 個地區頁，兩邊對不上。

**現在**：這不是「補資料填滿」的問題，而是整個機制換了（見 1.1）——時間軸節點跟「旅行足跡」的國家/地區數，現在完全由 `assets/js/trips.js` 的 `trips` 動態產生，不再手寫在 `index.html` 裡，5 筆假資料也已經全部移除。時間軸只反映 `trips` 裡真的有行程資料的旅程，**跟地區頁數量本來就不必一致**（有地區頁不代表有真實去過的行程資料）。之後每提供一趟新行程，時間軸會自動多一個節點，不需要再手動維護 `index.html`。
...
- [ ] **地區頁分類導覽在頁面捲動時固定顯示** — 目前地區頁往下捲動後，`.region-section-nav` 會跟著頁面內容移出視窗，使用者看不到分類導覽。修正後導覽列應在 Hero 捲出畫面後停留於 sticky header 下方，直到地區內容結束；桌面與手機版都不可遮住 header 或章節標題，手機版仍須保留導覽列的水平滑動。施工時檢查 `style.css` 的 sticky 定位、header 高度與祖先容器 overflow，並在 `tests/suites/region-content.mjs` 補上實際捲到頁面中後段仍可看見導覽列的回歸測試。
- [ ] **捲動頁面時顯示右下角「回到頂端」按鈕** — 頁面仍在頂端時隱藏，使用者往下捲動超過適當距離後才在視窗右下角淡入；點擊後將頁面平順捲回最上方，回到頂端後再次隱藏。按鈕使用原生 `<button>`、提供清楚的 `aria-label` 與鍵盤 focus 狀態，桌面、觸控與手機安全區都要可正常操作；`prefers-reduced-motion` 下改為立即回頂，不播放捲動或大幅位移動畫。樣式沿用全站暖色液態玻璃系統，以圓形半透明表面、backdrop blur、內側高光、低對比邊線、柔和陰影與暖橘色重點呈現，圖示沿用既有 Font Awesome，不使用厚重外框或突兀的純白底；hover／focus 可有輕微浮起與光暈，但不可遮住頁面主要控制項或 modal。實作時共用同一套 HTML／CSS／JavaScript，不為個別頁面複製邏輯，並補上顯示門檻、點擊回頂、鍵盤／觸控及 reduced-motion 的瀏覽器回歸測試。
