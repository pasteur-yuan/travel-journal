...
### 1.1 旅行筆記 — ✅ 模組已實作，全站 29 個地區的筆記內容目前都是空的、等你逐趟加回來
你提供「去趣行程總覽.md」（5 趟行程、304 列原始資料）後，模組與資料曾經一次全部做完，但你回報「呈現起來怪怪的」，決定資料全部清空重來、改成一趟一趟加，方便逐次確認。清空分兩輪，範圍最後擴及全站：

1. 這次新做的行程軌跡（`regionItineraries[地區][筆記標籤]`）——每個地區現在都是空物件 `{}`。
2. 7 個地區原本就有的第一則筆記文字（`regionContent[地區].note`）與其中 6 個地區的附加筆記（`regionAdditionalContent[地區].notes`）——這些是更早之前的階段寫的年月＋一段敘述文字，沒有行程軌跡資料支撐，你確認過也算「假資料」，兩個欄位都已從 JS 移除。
3. 你進一步指出，站上另外 22 個地區的原始 HTML 裡，`#notes` 章節其實一直都內建一則「待補充標籤＋一句敘述」的佔位筆記（例如京都「從東寺、清水寺到伏見的千本鳥居，把京都的參道與河川排成一段散步。」）——這則不是 JS 注入的，是每個地區頁 HTML 檔案裡寫死的 `.region-note`。你要求連這個也拿掉，於是把全部 29 個地區頁 HTML 裡的 `.region-note` 都刪除，只留章節標題（「JOURNAL / 旅行筆記」）與導覽連結，內容區暫時完全空白，等你逐趟給真實資料再加回去。

**模組本身（資料結構、modal 渲染、CSS、測試）維持完整保留，只有資料與 HTML 佔位內容被清空**：`regionContent`／`regionAdditionalContent` 不再有 `note`／`notes` 欄位，`regionItineraries[地區][筆記標籤]` 都是空物件，29 個地區頁的 `#notes` 章節底下也不再有任何 `.region-note` 或附加筆記卡片。畫面上「旅行筆記」標題會保留、底下留白，不是空白頁或壞掉，是刻意的過渡狀態。
...
測試維持 3 個群組：「旅行筆記卡片順序」「旅行筆記行程軌跡」「旅行筆記行程軌跡可及性」，全部改成先判斷有沒有筆記項目／行程資料可驗證，沒有就顯示「略過」並附上原因，不是硬點不存在的元素造成例外，也不是報一個沒真的驗證到東西的假通過。「地區頁 · Google Maps 連結」的「筆記顯示停留點清單」「筆記不顯示編造的資料表」兩項斷言同樣改成資料存在時才檢查，否則略過。全站測試 1193 通過、18 略過（略過的全部是這批跟筆記資料有關的斷言，等你補第一筆資料回來就會開始真正驗證，不是 bug）。
