...
- [ ] **手機版首頁世界地圖改為 marker 優先操作** — 手機 viewport 的世界地圖只保留已探索國家的可點擊 icon；時區多邊形與地圖底層不可拖曳、滑動或選取，手指在非 marker 區域上下滑動時必須由頁面正常垂直捲動，不可被地圖攔截。國家 icon 採兩段式觸控：第一次點擊只固定並顯示該國 tooltip，第二次點擊同一 icon 才轉導至國家頁；改點另一個 icon 時改顯示新 tooltip、不可誤導頁。桌面既有 hover／focus／直接連結行為及鍵盤可及性維持不變；需以手機 viewport 測試地圖手勢、兩段式 marker 點擊、轉導與 tooltip 切換。
