...
**狀態**：已收到 26 間飯店候選名單，尚未逐筆確認所在地、交通方式、是否仍營業與所屬地區；收錄範圍不限於原本的 22 個新地區。

執行規則：先以官方飯店／業者網站確認名稱與所在地，再依實際所在地判定應歸屬的現有地區頁，不受原本候選地區清單限制。每筆需寫進 `stayBaseContent[地區]`、`regionAdditionalContent[地區].stays` 與 `regionalVenueData[地區].stays`；modal 資料表維持「地名、資訊、交通方式、Google Map」四欄。若所在地沒有對應地區頁，必須新增該地區頁並同步補上國家頁清單、`tests/regions.mjs` 等既有地區定義後，再收錄住宿資料；不可臆測併入鄰近地區。完整原始名單見附錄 A.1。
...
- [ ] **首頁「已探索」與「旅行足跡」點擊切換收折** — 不論桌機、平板或手機，點擊地圖右上「已探索」或右下「旅行足跡」資訊框時，若目前為收折狀態則展開；再次點擊同一個資訊框則收折，並同步更新可及性狀態（如 `aria-expanded`）。兩個資訊框彼此的展開規則、既有 hover／focus 表現與 tooltip 避讓不得互相衝突；需測試滑鼠 click、鍵盤 Enter／Space 與觸控 tap 的連續開合。
