...
- [x] **手機版邊界拉動出現白底** — ✅ 程式面完成，**iOS Safari／Android Chrome 實機確認仍待你**。根因：全站只有 `body` 設了主題背景（`body.theme-glass`／`body.theme-glass-dark` 的漸層，以及國家頁專用的 `body.region-page` 手機深色覆寫），`html` 從未設背景；iOS 拉到邊界回彈時，瀏覽器畫的是 `html` 的背景，`html` 沒設就會透出預設白底。修法：`theme-switcher.js` 把 `theme-glass`／`theme-glass-dark`、`region-page` 這兩個 class 同步到 `document.documentElement`（html），`style.css` 把對應的背景規則從只認 `body.theme-glass` 擴大成 `html.theme-glass, body.theme-glass`（`theme-glass-dark` 同理），另外幫 `html.region-page` 補一條手機版深色覆寫規則，跟既有 `body.region-page` 對齊。**沒有**新增 `overscroll-behavior` 或關閉回彈——任務要求是回彈時維持主題色，不是取消回彈手感，也沒有動任何捲動容器，地區頁 section-nav、首頁卡片列、手機版時間軸的手勢邏輯完全沒碰。「地區頁」目前只有國家頁本身帶 `region-page` class（用於它的深色 hero），28 個地區詳細頁都沒有這個 class、維持一般主題背景——沿用既有規則，不是我新加的行為。在「全站 · 主題切換」測試裡加 9 項斷言：html／body 的主題 class、`region-page` 標記、computed background 三者是否一致，桌機三種頁面 × 手機版國家頁與地區頁都測；已用還原修正的方式確認測試會因為這個問題而變紅。無法在無頭 Chrome 裡重現 iOS 的橡皮筋回彈動態，只能驗證「html 背景永遠等於 body 背景」這個修正的正確性前提——這是 iOS 這個 bug 的根本機制，但視覺上還是需要你在真機上開亮／暗兩種主題、首頁／國家頁／地區頁分別拉到邊界確認。
...
- [ ] **地區頁 HTML 樣板化** — 28 個頁面骨架 99% 相同，但樣板化會加深對 JS 的依賴
