...
現況：29 個地區頁、514 列地點資料、1214 項測試通過

---

## 1. 內容資料

### 1.1 旅行筆記的行程軌跡 — ✅ 模組已實作，四趟行程資料已處理完成
你提供「去趣行程總覽.md」（5 趟行程、304 列原始資料）後，模組與資料同時完成。

**做法**：先用 Node 腳本把 markdown 表格解析成結構化資料（不是手動謄打 304 列，避免抄錯時間、地名），逐一比對每個地名屬於哪個地區（依實際地理位置，不是猜），輸出「地區 → 日期 → 停留點」的分組結果，人工檢查過每一筆的地區歸屬（尤其是同一天跨兩個地區的情況）後，才轉成最終的 `regionItineraries[地區][筆記標籤]` 寫入 `region-detail.js`。查表方式比照 `regionalVenueData`：用筆記卡的標籤（年 / 月）當 key，modal 改顯示時間軸式的「時間 → 地名 → 類型 → 備註 → 到下一站的交通方式」清單，不是原本四欄表格——沒有行程資料的筆記維持原樣（表格隱藏、不編造內容）。

**跨地區行程的處理**：同一趟旅程如果橫跨多個地區（例如 2025 年 4-5 月九州行程同時touch 熊本、宮崎、大分、福岡），每個地區的筆記只收錄屬於自己的停留點，不是整趟行程都塞進同一個地區；同一天如果跨兩個地區（例如熊本出發去宮崎高千穗再回熊本），也會依實際地點正確拆開。

**五趟行程分配結果**：
- 北海道 2026/01/01-01/09（單一地區）→ hokkaido「2026 / 01」，9 天 64 站
- 北海道 2024/11/29-12/07（單一地區）→ hokkaido「2024 / 12」，9 天 79 站
- 九州 2026/03/21-03/31（資料到 03/31，行程標題寫到 04/02——**這兩天還沒給資料**）→ fukuoka「2026 / 03」（附加到你已經寫好的既有筆記）11 天、oita「2026 / 03」1 天（湯布院）、saga「2026 / 03」1 天（鳥栖一站）
- 九州 2025/04/25-05/03（單一地區）→ kumamoto「2025 / 04」、miyazaki「2025 / 04」、oita「2025 / 04」、fukuoka「2025 / 05」
- 名古屋 2026/09/18-10/02（**只給了 09/18 當天 5 筆，09/19 之後完全沒有資料，你確認過會之後補**）→ nagoya「2026 / 09」，目前只有抵達日 1 天

kumamoto、miyazaki、saga 原本筆記標籤是「待補充」，這次直接補上正確的年月（描述文字沿用你之前已經寫好的內容，沒有改寫）；oita 原本完全沒有附加筆記，新增了一則。hokkaido、fukuoka、nagoya 各自新增了一則附加筆記。

**兩個已知缺口，需要你確認**：
- 唐戶市場（下關市，山口縣）出現在 2026/03/27 的行程裡，但站上目前沒有山口縣的地區頁，這一站先省略、沒有寫進任何地區——不是漏抓，是刻意不臆測併入福岡。之後如果山口有更多內容，可以比照 tochigi 的做法新增地區頁。
- 2026/03/23「746-1 Kanamaru」這一站前後都是博多的飯店，地名本身查不出明確地點，先低信心歸到 fukuoka；如果你知道實際地點，可以之後修正。

**過程中額外發現並修正一個既有 bug**：`#notes` 底下有兩個直接子 div（區塊編號「05」與實際內容），原本 `regionAdditionalContent[地區].notes` 的插入邏輯寫 `document.querySelector('#notes > div')` 只會抓到第一個（編號 div），導致新增的筆記全部插到編號 div 裡、排到第一則筆記（`regionContent[地區].note`）前面——這個 bug 影響原本就有附加筆記的全部 6 個地區（hokkaido／tokyo／nagoya／osaka／ise-shima／fukuoka），這次一併修正成抓 `.region-note` 的父層。

新增測試 3 個群組共 22 項：「旅行筆記卡片順序」驗證第一則筆記排在附加筆記之前（涵蓋全部原本受影響的地區，用還原修正的方式確認會變紅）、「旅行筆記行程軌跡」驗證天數／站數／跨地區拆分正確（北海道 9 天 64 站、宮崎只收錄屬於自己的 2 站）、「旅行筆記行程軌跡可及性」驗證鍵盤與觸控都能開啟。也更新了「地區頁 · Google Maps 連結」既有測試，把「筆記一律隱藏表格」的斷言拆成「沒有行程資料的筆記」與「有行程資料的筆記顯示停留點清單」兩種情況，過程中抓到一個真的存在的 bug：切到有行程資料的筆記時，`tbody` 沒有清空前一個項目留下的資料列（雖然表格本身是隱藏的、使用者看不到，但 DOM 裡確實殘留），已修正。全站測試 1214/1214 通過。
