...
// 這些測試需要至少兩個「有真實旅程、可以點開」的年份互相切換。真實 trips.js
// 目前只有一趟 2026 年的行程，隔壁的保留年份（currentYear + 1）沒有資料、
// marker 是 disabled 點不開，測不出年份切換行為，所以額外注入一筆隔年的合成
// 旅程，確保有兩個都能點開的年份。
const YEAR = new Date().getFullYear();
const extraTrip = tripScript({ date: `${YEAR + 1}-06-01`, country: "🇹🇹 測試國", regions: ["測試地區"] });
...
export const mobile = suite("時間軸 · 手機版互動", async (b, t) => {
  await b.mobile();
  await withTripVariant(b, extraTrip, async () => {
