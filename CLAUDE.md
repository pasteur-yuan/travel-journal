...
**卡片背景圖依 `regions[0]` 換成對應地區頁的主視覺**，不是整個日本共用一張圖：`main.js` 的 `regionCardImage(trip)` 用檔案頂端的 `regionImageSlugs`（中文地區名 → 地區頁主視覺檔名，**含副檔名**，例如 `'福井': 'fukui.svg'`）查出檔名，回傳可以直接指定給 CSS 的 `url("../images/<檔名>")` 字串；查不到對應項（地區還沒收錄，或非日本旅程）就退回 `japan.jpg`。副檔名不能省略或寫死 `.jpg`——新地區頁通常先是 `assets/images/<slug>.svg` 的漸層佔位圖，取得實拍照片後才連同 `style.css` 的 `.region-hero-<slug>` 一起換成 `.jpg`，兩處副檔名必須一致。**`url()` 要寫 `../images/...`，不能寫 `assets/images/...`**——CSS 自訂屬性裡的 `url()` 是相對「引用它的那條規則所在的樣式表」（`assets/css/style.css`）解析，不是相對 `index.html`，寫成從網站根目錄算的路徑會 404（真的踩過這個坑）。`regionImageSlugs` 是 `region-detail.js` 的 `regionNames`（方向相反）與 `tests/regions.mjs` 的 `REGION_NAMES` 之外，第三份需要同步的地區清單。
